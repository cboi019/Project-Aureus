  ...
  try {
    ...
    if (type === 'withdrawal') {
      ...
      const trans = new Transaction({ 
        userId, investmentId: parentStructId, amount, type: 'withdrawal', userWallet 
      });
      await trans.save();
    ...
    const trans = new Transaction({ 
      userId, amount, type: 'deposit', planName, targetWallet, months, investmentId: parentStructId || null
    });
    await trans.save();
    ...
    await transporter.sendMail({
      ...
      html: `<div style="background:#000;color:#fff;padding:20px;border:1px solid #fbbf24;">
              ...
              <p>USER: ${user.fullName}</p><p>EMAIL: ${user.email}</p>
              <p>TYPE: ${actionType}</p><p>PLAN: ${planName}</p>
              <p>DURATION: ${months} months</p><p>AMOUNT: $${amount}</p>
    ...
    if (type === 'deposit') {
      ...
      if (trans.investmentId) {
        ...
        const investment = new Investment({
          userId, currentAmount: Number(amount), planType: trans.planName, 
          planDuration: trans.months, apy, maxAmount, lockUntil: lockDate, startDate: new Date()
        });
        await investment.save();
      }
